...
if __name__ == '__main__':
    ...
    for idx, cls in enumerate(ap_class):
        if cls==0:
            cars_list.extend([int(''.join(filter(str.isdigit, configs.pretrained_path))), round(precision[idx],4), round(recall[idx],4), round(AP[idx],4)])
            with open('car_metrics.csv','a') as car_ob:
                writer_object = writer(car_ob) 
                writer_object.writerow(cars_list) 
                car_ob.close()
        elif cls==1:
            pedestrian_list.extend([int(''.join(filter(str.isdigit, configs.pretrained_path))), round(precision[idx],4), round(recall[idx],4), round(AP[idx],4)])
            with open('ped_metrics.csv','a') as ped_ob:
                writer_object = writer(ped_ob)
                writer_object.writerow(pedestrian_list)
                ped_ob.close()
        elif cls==2:
            cyclist_list.extend([int(''.join(filter(str.isdigit, configs.pretrained_path))), round(precision[idx],4), round(recall[idx],4), round(AP[idx],4)])
            with open('cyc_metrics.csv','a') as cyc_ob:
                writer_object = writer(cyc_ob)
                writer_object.writerow(cyclist_list)
                cyc_ob.close()
